...
def load_trust(file):
    df = pd.read_csv(file, sep = ' ', names=['user_id','user_id','rating'], dtype=np.float32)
    matrix = df.pivot_table(index='user_id', columns='user_id', values='rating')
    matrix.fillna(0, inplace=True)
    return matrix, np.count_nonzero(matrix)
...
if __name__ == "__main__":
    ...
    inputs_trust, num_trust = load_trust('trust.txt')
    # compatible matrix
    ...
    _, transformed_rating, transformed_trust = tdae(rating_vector, trust_vector, 3, [num_movies, 512, 256, 128, 256, 512, num_movies], 
                                                    [n_users, 512, 256, 128, 256, 512, n_users], 
                                                    ratio=0.75, active_function=tf.nn.relu)
    ...
    # train
    n_train_batches = (len(inputs_rating['train']['ratings']) // batch_size)
    cost = loss_op
