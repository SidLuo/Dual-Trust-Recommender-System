...
def load_matrix(file, random_state=42):
    ...
    train_val_indices, test_indices = train_test_split(indices, test_size=0.5, random_state=random_state)
...
def dae(inputs_rating, n_layer, shape_list_rating, ratio=0.5, activation_function = None):
    ...
    for i in range(n_layer):
        ...
        if i == 0:
            ...
            locals()[name_decoder_rating[i]] = activation_function(tf.matmul(hidden_layer, tf.transpose(last_weight) + bias))
        else:
            bias = tf.Variable(tf.constant(0.0, shape=[shape_list_rating[n_layer+i+1]], dtype=tf.float32))
            locals()[name_decoder_rating[i]] = activation_function(tf.matmul(locals()[name_decoder_rating[i-1]], tf.transpose(locals()[weights[n_layer-i-1]])+ bias))
